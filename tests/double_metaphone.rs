...
use nlp::phonetics::metaphone::double_metaphone::*;

const TEST_VALUES: &'static [&'static (&'static str, &'static str, &'static str)] = &[
    &("leçon", "LSN", "LSN"),
    &("ALLERTON", "ALRT", "ALRT"),
    &("Acton", "AKTN", "AKTN"),
    &("Adams", "ATMS", "ATMS"),
...
#[test]
fn double_metaphone_on_dict() {
    TEST_VALUES
        .iter()
        .foreach(|value| {
            let metaphone = match double_metaphone(&value.0) {
                Some(v) => v,
                None => panic!("hey")
            };
            println!("tuple: {:?} : (primary: {}, alternate: {}) ", value, metaphone.primary, metaphone.alternate);

            assert!(&metaphone.primary == &value.1);
